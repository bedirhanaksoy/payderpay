...
public class PaymentGatewayClient : IPaymentGatewayClient
{
    ...
    public PaymentGatewayClient(HttpClient httpClient, IOptions<ExternalServiceSettings> settings)
    {
    ...
    {
        var path = NormalizeRelativePath(_settings.PaymentProcessPath, "api/payment/process");
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            throw new HttpRequestException(
                $"Payment gateway request failed with status code {(int)response.StatusCode} for '{requestUri}'. Response: {Truncate(responseBody, 500)}",
                null,
                response.StatusCode);
        }
    ...
    private static Uri BuildRequestUri(Uri? baseAddress, string path)
    {
        ...
        {
            throw new InvalidOperationException("Payment gateway base URL is not configured.");
        }
